...
class VoxelIntensityDistribution:
    ...
    def calculate_vid(self, fiducial_values, x_array=None):
        density_local = self.integral(self.lum_func, 1e1, 1e8, args=(self.fiducial_values, self.fiducial_units))
        prob_1 = np.zeros(self.n_temp)
        prob_1[np.where(self.temp_range>0)] = self.vol_vox / (density_local * self.x_lt) * self.lum_func(self.temp_range[np.where(self.temp_range>0)] * self.vol_vox / self.x_lt,
                                                                            fiducial_values, self.fiducial_units)
        prob_n = self.prob_of_temp_given_n(prob_1, self.n_max)
        prob_signal = np.zeros(self.n_temp)
        for i in range(1, self.n_max + 1):
            prob_signal += prob_n[i] * self.prob_of_n_sources(i, density_local * self.vol_vox, fiducial_values[-1] ** 2)
        if self.mode == 'fft':
            prob_noise = 1.0 / np.sqrt(2 * np.pi * self.sigma_noise ** 2) \
                * np.exp(- self.temp_range ** 2 / (2 * self.sigma_noise ** 2))
            prob_total = self.prob_of_n_sources(0, density_local * self.vol_vox,fiducial_values[-1] ** 2) * prob_noise \
        ...
        if x_array is None:
            ...
            p_func = interpolate.interp1d(self.temp_range, prob_signal)#interpolate.splrep(self.temp_range, prob_signal, s=0)
    ...
    def prob_of_n_sources(self, n, mean_n, sigma_g_squared):
        return \
        integrate.quad(lambda mu: 1.0 / mu * self.prob_log_normal(mu / mean_n, sigma_g_squared) * self.poisson(n, mu),
